...
class ToDoListApp(App):
    ...
    def show_add_task_popup(self, instance):
        ...
        save_button.bind(
            on_release=lambda x: self.add_task(title_input.text, description_input.text, priority_spinner.text,
                                               due_date_button.text))
    ...
    def show_date_picker(self, button):
        ...
        save_button = Button(text='Save', size_hint_y=None, height=50)
        save_button.bind(
            on_release=lambda x: self.set_due_date(button, year_spinner.text, month_spinner.text, day_spinner.text,
                                                   hour_spinner.text, minute_spinner.text))
        content.add_widget(save_button)
    ...
    def show_edit_task_popup(self, task):
        ...
        save_button = Button(text='Save', size_hint_y=None, height=50)
        save_button.bind(
            on_release=lambda x: self.edit_task(task, title_input.text, description_input.text, priority_spinner.text,
                                                due_date_button.text))
        content.add_widget(save_button)
